...
h_N=1.46; %stała h_N - parametr wygładzania
temp_f_x=result(1:N); 
iter=1;
f_x=zeros(1,2001);
...
for x=-10:0.01:10
    ...
    for i=1:N
        %Jądro Gaussa
        %sum = sum + (1/sqrt(2))*exp(-((result(i)-x)/h_N)^2/2);
...
%         end
        ...
        %jądro Epanechnikova
        if (result(i)-x)/h_N < 3/4*(1-((result(i)-x)/h_N)^2) && (result(i)-x)/h_N > -3/4*(1-((result(i)-x)/h_N)^2)
            sum = sum + 1;
        end  
    end
    f_x(iter) = sum / (N*h_N);
